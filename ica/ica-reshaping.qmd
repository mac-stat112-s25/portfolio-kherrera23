...
The order matters, because the proportions change regarding a comparison between all species that are female or those from one species that are female and male.
...
Yes, we lost information about the bill and flipper measurements. We also don't know how many penguins from each combo were consider to calculate the average.
...
`ggplot(sleep_wide, aes(y = reaction_time, x = days_of_sleep, color = subject))`
...
# For cols, try 2 appproaches: using - and starts_with
 sleep_wide |> 
  pivot_longer(cols = starts_with("day"), names_to = "day", values_to = "raction_time") |> head(3)
```
...
-   Besides putting each argument on a different line for readability and storing the results, what changed in the code? it was added names_prefix = "day_"
-   How did this impact how the values are recorded in the `day` column? it kept the day number without the prefix day_
...
> subject is an integer and day is a character. We want them to be categorical and numerical, respectively.
...
ggplot(sleep_long, aes(y = reaction_time, x = day, colour = Subject)) +
  geom_line()
...
> Reaction time increases with a lack of sleep. Some subject seem to be more impacted than others by lack of sleep, and some tend to have faster, slower reactions times in general.
...
Adjust the code below. What don't you like about the column labels? they don't indicate what the numbers mean (missing day_)
...
nov_1999 |>
  ...
  ggplot(aes(y=ranking, x = week, colour = artist)) +
  geom_line()
```

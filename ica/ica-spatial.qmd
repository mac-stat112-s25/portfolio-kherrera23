...
# We need to indicate the data set we are using in the geom_point
ggplot(world_boundaries) + 
  geom_sf() + 
  geom_point(
    data = starbucks,
...
We learn the locations of starbucks around the world at the time the data set was recorded. It is more common in North America, Europe and east Asia.
...
```{r}
# There is not data points but regional density
# We change the geom from point to density 2d
ggplot(cma_boundaries) + 
  geom_sf() + 
  geom_density_2d(
    data = starbucks_cma,
...
ggplot(elections_by_counties, aes(map_id = county_fips, fill = median_rent)) +
  geom_map(map = county_map) +
  scale_fill_gradientn(colors = rev(brewer.pal(10, "RdBu")), name = "% Median Rent") +
  expand_limits(x = county_map$long, y = county_map$lat) +
  theme_map() +
  theme(legend.position = "right") + 
  coord_equal()
...
ggplot(elections_by_counties, aes(map_id = county_fips, fill = median_age)) +
  geom_map(map = county_map) +
  scale_fill_gradientn(colors = rev(brewer.pal(10, "RdBu")), name = "% Median Age") +
  expand_limits(x = county_map$long, y = county_map$lat) +
  theme_map() +
  theme(legend.position = "right") + 
  coord_equal()

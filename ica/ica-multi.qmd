...
ggplot(weather, aes(y = temp3pm, x = date, color = location, group = location)) + 
...
> The range seems to be from around 800 to 1100. The higher point is around 900 and then there is a decrease in bell shape form, this is the typical outcome. There is a second bell shaped curve between 1000 and 1100, therefore is bi-modal. This shape could be explained by the spending per pupil invested. 
...
```{r eval = FALSE}
ggplot(education, aes(y = sat, x = salary, color = cut(expend, 4))) + 
  geom_point() + 
...
> States with lower expend per pupil and teacher's salary seems to have a better perfomance in the SAT.
...
> States with lower spending seems to have higher scores because the students who choose to take the SAT are lower than in other states. 

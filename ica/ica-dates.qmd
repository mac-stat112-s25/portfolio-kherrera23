...
penguins |>
  mutate(body_mass_kg = body_mass_g/1000) |> head()
```
...
- Sort the species by groups
- mutate bill length from mm to cm 
- Arrange descending bill length in cm 
...
penguins |>
  select(species, ends_with("mm")) |> head()
...
penguins|>
  select(species, starts_with("bill")) |>head()
```
...
```{r}
# These lines gives you the month, the first by number and the second by name because includes label = TRUE
month(today)
month(today, label = TRUE)
```

```{r}
# The number of the week of the year out of 52
week(today)
```

```{r}
# The first line is the number day of the month 
# The second line is the number day of the whole year, out of 365
mday(today)
yday(today)  # This is often called the "Julian day"
```

```{r}
# The first line is the day of the week in numbers
# The second line is the day of the week by name.
wday(today)
wday(today, label = TRUE)
```

```{r}
# In the first row, today is a day higher than the date
# In the second row, today is lower than the date
...
```{r}
monthly_birthdays <- Birthdays |>
  mutate(month_name = month(date, label = TRUE)) |>
  group_by(year, month_name) |>
  summarise(total_births = sum(births))

ggplot(monthly_birthdays, aes(x = month_name, y = total_births)) +
...
ggplot(weekly_births_1988, aes(y = total_births, x = week_of_year, colour = state)) +
  geom_line()
```
...
ggplot(MN_LA_births, aes(x = week_of_year, y = total_births, color = state)) +
  geom_line()

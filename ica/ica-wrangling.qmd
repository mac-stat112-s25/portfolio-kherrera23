...
a.  keep *only* information about state names, county names, and the 2020 and 2016 Democratic support (not the 2012 results, demographics, etc) - Select

b.  get *only* the data on Minnesota - Filter

c.  define a new variable which calculates the *change* in Democratic support from 2016 to 2020, using `dem_pct_20` and `dem_pct_16` - mutate

d.  sort the counties from highest to lowest Democratic support - group-by

e.  determine the total number of votes cast across all counties - summarize
...
```{r}
# Keeps data from Hawaii and Delaware
elections_small |> 
...
```{r}
# Created a new column with the difference between repub and dem support in the 2020 election
elections_small |> 
...
```{r}
# Define repub_win_20, wheter republican won in the 2020 election or not.
elections_small |> 
...
The counties in Wisconsin where the support of democrats was higher than republican, arrange in a higher to lower order
...
BEFORE running the below chunk, what do you think it will produce? 
Rows with counties in Delaware with a column call repub_win_20 that shows wheter Republican had more support than Democrats. Only this columns will be part of the visualization: county_name, repub_pct_20, dem_pct_20, repub_win_20
...
mn_wi <- elections_small |>
  ...
  select(state_name, dem_pct_20, dem_pct_16) |>
...
```{r}
# There was a stronger Dem chnage from 2016 to 2020 in Minnesota
...
```{r}
# Calculate the median of Republican support in 2020 accross all counties
elections_small |> 
  summarize(median(repub_pct_20))
```

```{r}
# Calculate the median of Republican support in 2020 accross all counties and name it 'median_repub'
 elections_small |> 
   summarize(median_repub = median(repub_pct_20))
```

```{r}
# calculates the median of republican support across all counties, name it "median_repub" and calculate the toal votes of the election across all counties.
 elections_small |> 
...
```{r}
# calculate the republican median and the total votes per state
# we would get the results by county
elections_small |> 
   group_by(state_name) |> 
   summarize(median_repub = median(repub_pct_20), total_votes = sum(total_votes_20)) 
```
...
# - sort the states -> group-by
# total votes in 2020 -> summarize
# - from the most to least total votes cast in 2020 -> arrange(desc())
...
# In each state -> group-by
# Total number of votes for dem -> summarize
# Total number of votes dro Rep -> summarize

elections_small |>
  mutate(dem_votes_20 = round(total_votes_20*dem_pct_20/100), repub_votes_20 = round(total_votes_20*repub_pct_20/100)) |>
  group_by(state_name) |>
...
# what states-> group-by
# democratic candidate win in 2020 -> mutate

elections_small |>
  group_by(state_name) |>
  mutate(dem_won_20 = dem_pct_20 > repub_pct_20)
```
...
elections_small |>
  mutate(repub_total_votes = round(repub_pct_20*total_votes_20/100), dem_total_votes = round(dem_pct_20*total_votes_20/100)) |>
  group_by(state_name) |>
  summarise(statrepb_total = sum(repub_total_votes), 
            statdem_total = sum(dem_total_votes))
```
...
elections_small |>
  ...
  summarise(statrepb_total = sum(repub_total_votes), 
            statdem_total = sum(dem_total_votes)) |>
  filter(statdem_total > statrepb_total)
```

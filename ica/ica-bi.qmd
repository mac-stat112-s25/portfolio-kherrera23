...
# How many students have now filled out the survey? 28 students filled out the survey 

# What type of variables do we have? quantitative and qualitative
...
ggplot(survey, aes(x = hangout)) +
  labs( x = "Place of hangout", y = "Students") +
...
# How does this differ than the frame for our histogram of repub_pct_20 alone? It adds a y-axis.
...
# What happens if you change the shape to another number? It changes the shape of the data points.
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16)) +
  geom_point(shape = 3)
```
...
```{r eval = FALSE}
# the colors don't alling with the name of the category
...
```{r eval = FALSE}
# The scale_fill_manual defines which colors are use in the plot
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
...
```{r eval = FALSE}
# Alpha = 0.5 make the fill transparent
# The closer alpha is to 0 the more transparent and the closser is to 1, the more opaqueit is.
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
...
```{r eval = FALSE}
# It separates the density plots in facets for each historical group
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
  scale_fill_manual(values = c("blue", "purple", "red")) +
...
# It is hard to understand and find patterns
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
...
- Pro: easier to compare the relative outcome in blue vs purple vs red states
- Con: we don't know how many counties are part of each category.

#### Part b 

What's your favorite bar plot from part and why?
- the stacked bar plot because i find it easier to understand in context of the data and identify trends 

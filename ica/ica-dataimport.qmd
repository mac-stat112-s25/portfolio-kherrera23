...
> The data was found in kaggle and is originally from The Movie Database (TMDb)
...
```{r}
library(tidyverse)
imdb_messy <- read_csv("~/Documents/GitHub/portfolio-kherrera23/data/imdb_5000_messy.csv")
```
...
-   What's goofy about this / what do we need to fix? There are 5 categories for something that could be two.
-   More specifically, what different categories does the `color` variable take, and how many movies fall into each of these categories? The way the variable is writen affects the counting.
...
imdb_messy |> 
  summarise(num_NAs = sum(is.na(actor_1_facebook_likes)))
```
...
> We remove the NAs from the calculations. Not it was not need it a new data set.
...
-   What did `drop_na()` do? How many data points are left? Dropped all the data points with missing information in any column, now we have 3756 rows.
-   In what situations might this function be a good idea? When need it to use all the variables 
-   In what situations might this function be a bad idea? If you are just working with certain column where all movies have it and you lose some rows fro missing information in a variable that do not affect your project.
